...
from app.schemas.order_item import OrderItemCreate, OrderItemUpdate, OrderItemOut
...
@router.post(
    "/",
    response_model=ResponseModel[OrderItemOut],
...
def create_order_item(order_item_create: OrderItemCreate, db: Session = Depends(get_db)):
    ...
    return ResponseModel(
        data=OrderItemOut.model_validate(new_order_item),   # ← ORM → Pydantic
...
@router.get(
    "/order/{order_id}",
    response_model=ResponseModel[list[OrderItemOut]],
...
def get_order_items(order_id: int, db: Session = Depends(get_db)):
    ...
    return ResponseModel(
        data=[OrderItemOut.model_validate(i) for i in order_items],  # ← list conversion
...
def get_order_item(item_id: int, db: Session = Depends(get_db)):
    ...
    return ResponseModel(
        data=OrderItemOut.model_validate(order_item),       # ← ORM → Pydantic
...
def update_order_item(item_id: int, order_item_update: OrderItemUpdate, db: Session = Depends(get_db)):
    ...
    return ResponseModel(
        data=OrderItemOut.model_validate(updated_order_item),  # ← ORM → Pydantic
...
@router.post(
    "/bulk/add",
    response_model=ResponseModel[list[OrderItemOut]],       # ← list not single
    dependencies=[Depends(require_permission("orders.create"))]
)
def add_bulk_items(items: List[OrderItemCreate], db: Session = Depends(get_db)):
    ...
    for item_in in items:
        ...
        added_items.append(OrderItemOut.model_validate(item))  # ← convert each item
    return ResponseModel(
        data=added_items,
